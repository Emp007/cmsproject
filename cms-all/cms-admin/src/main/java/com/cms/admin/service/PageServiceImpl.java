...
public class PageServiceImpl implements PageService {
	...
	public Meta Save(Meta meta) {
		if(meta==null){
			throw new CMSAdminException(String.format("page cannot be null "));
		}
		int status = 0;
		try {
			JsonNode response = restServiceUtil.makeRequest(URLConstants.UPDATE_PAGE_BY_PAGE_ID, meta , null, HttpMethod.POST);
			status = response.get(CMSConstant.STATUS_CODE).intValue();
			if (status != 200) {
				throw new CMSAdminException(String.format("API not responded while saving page by page id."));
			}
			String data = response.get(CMSConstant.DATA).toString();
			return OBJECT_MAPPER.readValue(data,new TypeReference<Meta>() {
					});
		} catch (Exception e) {
			logger.error("Error while saving page by page id", e);
			throw new CMSAdminException("Error while saving page by page id", e);
