(function(c) {
	...
	b.prototype = {
		...
		input : function(d) {
			if (d === true) {
				if (!this.options.input) {
					...
					this.$elementFilestyle.find(".quant-files-filestyle")
							.remove();
					var e = "", g = [];
						...
						if (g.length > 0) {
							...
							this.$elementFilestyle
									...
									.append(
											" <span "
													+ h
													+ ' class="quant-files-filestyle badge">'
													+ g.length + "</span>")
						}
					}
				} else {
		...
		constructor : function() {
			...
			if (this.options.buttonBefore) {
				g = '<label for="' + h + '" style="margin-right: 4px;" class="'
						+ this.options.classButton + '" '
						+ (this.options.disabled ? 'disabled="true"' : "")
						+ ">" + this.htmlIcon() + "<span>"
						+ this.options.buttonText + "</span></label>"
						+ this.htmlInput()
			} else {
				g = this.htmlInput() + '<label for="' + h + '" class="'
						+ this.options.classButton + '" '
						+ (this.options.disabled ? 'disabled="true"' : "")
						+ "> " + this.htmlIcon() + "<span>"
						+ this.options.buttonText + "</span></label>"
			}
			this.$elementFilestyle = c('<div class="bootstrap-filestyle" style="display: inline-block;">'
					+ g + "</div>");
					...
					.change(function() {
						...
						if (i.options.input == false) {
							...
							if (i.$elementFilestyle
								...
								if (d.length == 0) {
									i.$elementFilestyle.find(
											".quant-files-filestyle").remove()
								} else {
									i.$elementFilestyle.find(
											".quant-files-filestyle").html(
											d.length)
								}
							}
						} else {
							i.$elementFilestyle.find(".quant-files-filestyle")
									.remove()
						}
					});
